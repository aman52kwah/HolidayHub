...
const CountrySelector = ({countries,selectedCountry,
    isLoading,onSelectCountry
}) =>{
    const handleChange =(e)=>{
        onSelectCountry(e.target.value);
    };

    const selectedCountryName = countries.find(c => c.isoCode === selectedCountry)?.name ||'Loading...';
        ...
        <div className="relative w-full max-w-xs">
            <label htmlFor="country-select" className="block text-sm font-medium
             ...
             <p className="mt-1 text-sm text-gray-500">
                {isLoading ? 'Loading availabe countries...' :
